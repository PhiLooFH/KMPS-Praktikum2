...
object Main {
  ...
  private def getStringContent(charList: List[Char], returnString: String) : String = charList match {
    case '>'::_ => returnString //text starts with < and ends with > - Tag
    case '<'::_ => returnString //text starts with > and ends with < - Content
    case x::xs => getStringContent(xs, returnString + x)
    case _ => getStringContent(charList.tail, returnString)
  }
